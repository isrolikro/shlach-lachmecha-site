...
export class DriveDataService {
  ...
  public static readonly CATEGORIES_CONFIG: Record<string, string[]> = {
    ...
    'במדבר': ['במדבר', 'נשא', 'בהעלותך', 'שלח', 'קורח', 'חקת', 'בלק', 'פנחס', 'מטות', 'מסעי'],
  ...
  private parseFileName(file: any): Lesson {
    ...
    const titleClean = cleanName
      .replace(/\s*ה['״"]\s*תשפ[א-ת"״'_]*/g, '') // ה'תשפ"ו
      .replace(/\s*תשפ[א-ת"״'_]*/g, '')            // תשפ"ו / תשפ'ו / תשפו
      .replace(/\s*\d{4}/g, '')                      // 2024
      .replace(/\s+[א-ת]['״"]\s*$/g, '')             // ה' שנשאר בסוף
      .trim();
